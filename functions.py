...
def sortbypubs(data,sortbyauth,data_fauthpub):
    L2 = len(data)
    multpubs = []
    num_pairs = []
    count = 0
...
def measuresofquality(stdevtot,data):
    # Mean unsigned error
    L = len(data)
...
def findpubs(data):
    ...
    data['Uniq Publication Title'] = title_lists_pub
...
def make_groups(data,MRC,IsLogTransf,P,cor_units):
    ...
    for i in range(L):
        ...
        ind_klf = example.columns.get_loc('kineticlaw_formula')
        formulas.append(list(example.iloc[0,ind_klf]))
        ...
        if cor_units == False:
            ...
            klt.append(list(example['kineticlaw_type']))
    ...
    if cor_units == False:
        ...
        grp['kineticlaw_type'] = pd.Series(klt)
    ...
    #dat['formula'] = [x[0] for x in dat['formula']]
    return dat
